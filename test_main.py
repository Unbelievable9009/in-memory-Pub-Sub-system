...
from main import MAX_HISTORY, MAX_SUBSCRIBER_QUEUE, app, topics
import pytest
...
async def test_stats():
  ...
  assert response.json() == {
      "topics": {"stats_topic": {"messages": 0, "subscribers": 0}}
  }

  # Publish a message and check stats again
  ...
  assert response.status_code == 200
  assert response.json() == {
      "topics": {"stats_topic": {"messages": 1, "subscribers": 0}}
  }
...
def test_websocket_topic_not_found():
  with client.websocket_connect("/ws") as websocket:
    websocket.send_json(
        {"type": "subscribe", "topic": "nope", "client_id": "c1"}
    )
    response = websocket.receive_json()
    assert response["type"] == "error"
...
async def test_websocket_pubsub():
  ...
  with (
    ...
    with client.websocket_connect("/ws") as publisher:
      ...
      assert pub_ack["topic"] == "pubsub"
      assert pub_ack["status"] == "ok"
    ...
    assert unsub_ack1["topic"] == "pubsub"
    assert unsub_ack1["status"] == "ok"
    ...
    with pytest.raises(
        Exception
    ):  # Adjust exception type based on testing client behavior on timeout
...
async def test_websocket_last_n():
  ...
  with client.websocket_connect("/ws") as subscriber:
    ...
    subscriber.send_json({
        ...
        "client_id": "s-lastn",
        "last_n": 3,
        "request_id": "sub-lastn",
    })
    ack = subscriber.receive_json()
    assert ack["type"] == "ack"
    assert ack["request_id"] == "sub-lastn"
    assert ack["topic"] == "lastn"
...
async def test_websocket_backpressure():
  ...
  with client.websocket_connect("/ws") as subscriber:
    ...
    ack = subscriber.receive_json()
    assert ack["type"] == "ack"
    assert ack["request_id"] == "sub-slow"
    assert ack["topic"] == "backpressure"
    assert ack["status"] == "ok"
    ...
    for i in range(MAX_SUBSCRIBER_QUEUE):
      await queue.put({
          "type": "event",
          "message": {"id": str(uuid.uuid4()), "payload": {"i": i}},
      })
    assert queue.full()
    ...
    if 0 in received_messages:
      print(
          "Oldest message (i=0) was not dropped, received:", received_messages
      )
    ...
    try:
      ...
      while True:
        ...
        if (
            msg["type"] == "event"
            and msg["message"]["payload"].get("data") == "overflow"
        ):
          has_overflow = True
          break
    except Exception:
...
async def test_topic_deletion_notifies_subscribers():
  ...
  with client.websocket_connect("/ws") as subscriber:
    ...
    ack = subscriber.receive_json()  # ACK
    assert ack["type"] == "ack"
    assert ack["topic"] == "to_be_deleted"
    assert ack["request_id"] == "sub-del"
    assert ack["status"] == "ok"
    await asyncio.sleep(0.01)
