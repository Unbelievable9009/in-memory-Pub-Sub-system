...
class Topic:
    ...
    async def add_subscriber(self, websocket: WebSocket, last_n: int = 0):
        async with self.lock:
            if websocket not in self.subscribers:
                ...
                self.subscriber_queues[websocket] = asyncio.Queue(maxsize=MAX_SUBSCRIBER_QUEUE)
    ...
    async def _enqueue_message(self, websocket: WebSocket, message: Dict[str, Any]):
        ...
        try:
            if queue.full():
                ...
                try:
                    queue.get_nowait()
                    queue.task_done()
                    print(f"Warning: SLOW_CONSUMER - Dropped oldest message for client on topic {self.name}")
                    ...
                    error_msg = {
                        ...
                        "error": {
                            "code": "SLOW_CONSUMER",
                            "message": "Subscriber queue overflow, oldest message dropped."
                        },
                    ...
                    try:
                         ...
                         print(f"Warning: Could not send SLOW_CONSUMER error to client on topic {self.name} without blocking.")
        ...
        try:
            while websocket in self.subscribers:
                ...
                try:
                    ...
                    print(f"Error sending message to subscriber on topic {self.name}: {e}")
...
async def create_topic(topic_name: str) -> Topic:
    async with topics_lock:
        if topic_name in topics:
            raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail=f"Topic '{topic_name}' already exists")
...
async def delete_topic(topic_name: str):
    async with topics_lock:
        ...
        if not topic:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"Topic '{topic_name}' not found")
    ...
    try:
        while True:
            ...
            if not topic_name and msg.type in ["subscribe", "unsubscribe", "publish"]:
                await send_error(websocket, "Missing 'topic'", "BAD_REQUEST", request_id)
                continue

            topic = await get_topic(topic_name)

            if not topic and msg.type in ["subscribe", "publish"]:
                await send_error(websocket, f"Topic '{topic_name}' not found", "TOPIC_NOT_FOUND", request_id, topic=topic_name)
                continue

            try:
                if msg.type == "subscribe":
                    if not msg.client_id:
                        await send_error(websocket, "Missing 'client_id' for subscribe", "BAD_REQUEST", request_id, topic=topic_name)
                        continue
                    await topic.add_subscriber(websocket, msg.last_n)
                    client_subscriptions.add(topic_name)
                    await send_ack(websocket, request_id, topic_name, "Subscribed")

                elif msg.type == "unsubscribe":
                    if not msg.client_id:
                        await send_error(websocket, "Missing 'client_id' for unsubscribe", "BAD_REQUEST", request_id, topic=topic_name)
                        continue
                    if topic:
                        await topic.remove_subscriber(websocket)
                    client_subscriptions.discard(topic_name)
                    await send_ack(websocket, request_id, topic_name, "Unsubscribed")

                elif msg.type == "publish":
                    if not msg.message:
                        await send_error(websocket, "Missing 'message' for publish", "BAD_REQUEST", request_id, topic=topic_name)
                        continue
                    try:
                        # Validate message ID
                         uuid.UUID(msg.message.id)
                    except ValueError:
                         await send_error(websocket, "message.id must be a valid UUID", "BAD_REQUEST", request_id, topic=topic_name)
                         continue
                    ...
                    await send_ack(websocket, request_id, topic_name, "Published")

                else:
                    await send_error(websocket, f"Unknown message type '{msg.type}'", "BAD_REQUEST", request_id)

            except HTTPException as e: # Catch errors from topic operations
                await send_error(websocket, e.detail, "BAD_REQUEST", request_id, topic=topic_name)
...
async def send_error(websocket: WebSocket, message: str, code: str, request_id: Optional[str] = None, topic: Optional[str] = None):
    error_msg = {
        "type": "error",
        "request_id": request_id,
...
async def send_ack(websocket: WebSocket, request_id: Optional[str], topic: str, status_msg: str):
    ack_msg = {
        "type": "ack",
        "request_id": request_id,
        "topic": topic,
...
async def http_list_topics():
    async with topics_lock:
        topic_list = [{"name": name, "subscribers": len(topic.subscribers)} for name, topic in topics.items()]
